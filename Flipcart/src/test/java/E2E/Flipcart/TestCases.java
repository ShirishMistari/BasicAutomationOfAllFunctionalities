...
public class TestCases extends ActionsOnElements implements Values {
	...
	public void e2e() throws InterruptedException, IOException {
		WebDriver driver1 = i.commonLoginPage(driver);
		WebElement searchInput = clickOnElement(driver1, inputXpath);
		searchInput.sendKeys(input, Keys.ENTER);
		CommonWait.waitForElementsToBeVisible(driver1, k);
		...
		for (WebElement listP : products) {
			try {
				...
				if (p.contains(productName)) {
					...
					WebElement productFound = listP.findElement(kk);
					productFound.click();
					break;
				}
			} catch (Exception e1) {
		...
		Thread.sleep(1000);
		CommonWait.waitForWebElementBy(driver1, oo);
		WebElement addToCart = webElement(driver1, addTocart);
		JavascriptExecutor js = (JavascriptExecutor) driver1;
		js.executeScript("arguments[0].scrollIntoView(true);", addToCart);
		addToCart.click();
		CommonWait.waitForWebElementBy(driver1, pp);
